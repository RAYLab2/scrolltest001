...
// 이미지 경로 배열 생성
for (let i = 1; i < 22; i++) {
  if (i < 10) $avataImgSqc.push(`layer/Layer00${i}.jpg`);
  else $avataImgSqc.push(`layer/Layer0${i}.jpg`);
}
...
smscene.on("progress", function (event) {
  ...
  if (text[text.length - 1] == "Layer021.jpg") {
...
function slickHover(id) {
  if (IsScrollEnd) showImage(id, false);
}
